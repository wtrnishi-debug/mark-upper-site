/* Mark Upper – bookmarklet overlay */
(function () {
  'use strict';
  ...
  // 二重起動ガード
  if (window.__mu_bm) { window.__mu_bm.toggle(); return; }
  ...
  window.__mu_bm = { toggle: togglePanel };
  ...
  async function boot() {
    ...
    // セッション取得 or 作成
    const { data: ex } = await mu.sb
      .from('mu_sessions').select('*').eq('domain', mu.domain)
      .order('created_at', { ascending: false }).limit(1).single();
    mu.session = ex;
    if (!mu.session) {
      const { data } = await mu.sb
        .from('mu_sessions').insert({ domain: mu.domain }).select().single();
      mu.session = data;
    }
    if (!mu.session) { toast('セッションの作成に失敗しました'); return; }

    // コメント取得
    const { data: comments } = await mu.sb
      .from('mu_comments').select('*')
      .eq('session_id', mu.session.id)
      .eq('page_url', mu.pageUrl)
      .is('parent_id', null)
      .order('created_at');
  ...
  // ── ピン layer（ページ全体にかぶせる absolute div）──────
  function buildLayer() {
...
.body { background: #1a1a1a; border: 1px solid #2e2e2e; border-radius: 12px;
  padding: 12px; width: 196px; display: none;
...
<div class="panel" id="panel">
  ...
  function onOverlayClick(e) {
    if (!mu.annotating || !mu.session) return;
  ...
  function openForm(xPct, yPct, cx, cy) {
    ...
    const pop = makePop(`
      <div class="mu-hdr">
        <span>新しいコメント</span>
        <button class="mu-x" id="mu-fx">✕</button>
      </div>
  ...
  async function submitComment(xPct, yPct) {
    ...
    if (data) { mu.comments.push(data); addPin(data); refreshPanel(); }
    closePop();
  }
  ...
  function openDetail(c, cx, cy) {
    ...
    const pop = makePop(`
      ...
      <div class="mu-body">${esc(c.text)}</div>`);
    placePop(pop, cx, cy);
...
#__mu_overlay.__mu_active {
  pointer-events: all !important; cursor: crosshair !important;
...
.__mu_pop .mu-ta {
  width: 100% !important; background: #111 !important; border: 1px solid #333 !important;
...
.__mu_pop .mu-cancel {
  background: #2a2a2a !important; border: 1px solid #333 !important; border-radius: 6px !important;
...
.__mu_pop .mu-submit {
  background: #0099ff !important; border: none !important; border-radius: 6px !important;
